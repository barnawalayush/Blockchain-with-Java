...
class MultifunctionalMapper {
    ...
    public static final UnaryOperator<List<Integer>> IDENTITY_TRANSFORMATION =
            MULTIFUNCTIONAL_MAPPER.apply(List.of(x -> x));
    ...
    public static void main(String[] args) {
        ...
        List<Integer> idTransformation = IDENTITY_TRANSFORMATION.apply(numbers);

        List<Integer> mult2AndAdd1Mapper = MULTIFUNCTIONAL_MAPPER
                .apply(Arrays.asList(x -> x * 2, x -> x + 1))
                .apply(numbers);

        List<Integer> mult2AndAdd1Transformation = MULT_TWO_AND_THEN_ADD_ONE_TRANSFORMATION.apply(numbers);

        List<Integer> squareAndNextEvenMapper = MULTIFUNCTIONAL_MAPPER
                .apply(Arrays.asList(x -> x * x, x -> x % 2 == 0 ? x + 2 : x + 1))
                .apply(numbers);

        List<Integer> squareAndNextEvenNumberTransformation = SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION.apply(numbers);

        StringBuilder result = new StringBuilder("")
                .append(getStringFromList(idMapper))
                .append(TAB)
                .append(getStringFromList(idTransformation))
                .append(TAB)
                .append(getStringFromList(mult2AndAdd1Mapper))
                .append(TAB)
                .append(getStringFromList(mult2AndAdd1Transformation))
                .append(TAB)
